...
#generate the same grain structure with different resolutions
geom192 = damask.Grid.from_Voronoi_tessellation(np.ones(3,dtype=int)*192,
                                                np.ones(3),
                                                np.random.random((N,3)))
for g in (96, 64, 32):
    geom = damask.Grid(geom192.material[::(192//g),::(192//g),::(192//g)], np.ones(3))
    geom = geom.canvas(cells = geom.cells+np.array([0,20,0]))
    geom.save(f'{N}_{g}')
...
#simulation
for g in (96, 64, 32):
    os.system(f'time mpirun -n 16 DAMASK_grid --load tensionX.yaml --geom {N}_{g}.vti > {N}_{g}.out')
    result = damask.Result(f'{N}_{g}_tensionX.hdf5')
    np.savetxt(f'{N}_{g}_slip.txt', result.place('gamma_sl')[f'increment_50'].__array__(), fmt='%.4e')
    os.system(f'rm {N}_{g}_tensionX.hdf5')
